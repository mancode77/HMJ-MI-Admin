    ...
    async function getData() {
      const semester = await db.collection('smt5').get();
    ...
    document.getElementById('data').addEventListener('click', function(e){
        ...
        if (e.target.classList.contains('hapusData')) {
            if (confirm("Apakah Anda Ingin Menghapusnya?")) {
              const dosen = db.collection('smt5').doc(id);
              dosen.get().then((doc) => {                   // Function Delete IMG In Storage Start
                if(doc.exists){
                  storage.ref().child(`smt5/${doc.data().nama}.jpg`).delete();
    ...
    document.getElementById('updateData').addEventListener('click', function (){
      db.collection('smt5').doc(document.getElementById('id').value).update({
    ...
    document.getElementById('data').addEventListener('click', function(e) {
      ...
      if (e.target.classList.contains('editData')) {
        db.collection("smt5").doc(id).get().then((doc) => {
            if(doc.exists){
    ...
    document.getElementById("saveData").addEventListener("click", function () {
      ...
      function addImg(){
        ...
        const fileRef = storage.ref(`smt5/${document.getElementById('inputNama').value}.jpg`);
